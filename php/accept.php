...
if(isset($_SESSION['password']) AND isset($_SESSION['user']) AND isset($_POST['reciver'])){
	...
	if($load){?>
		<?php 
		$Contact_tab = $bdd->prepare('SELECT IdAccount, ProfilPictureFile FROM Account WHERE Pseudo=:idcontact');
		$Contact_tab->execute(array('idcontact'=>$reciverid));
		if($Contact=$Contact_tab->fetch()){
			$reciver = $Contact['IdAccount'];
			$request = $bdd->prepare('UPDATE Contact SET Approval = TRUE WHERE IdAccount=:idaccount AND IdAsker=:idcontact');
			$request->execute(array('idaccount'=>$User["IdAccount"], 'idcontact'=>$reciver));
			echo $reciver.'/'.$Contact['ProfilPictureFile'];
		}
	}
}
?>
